...
function sendSlack($text, $receiver) {
    ...
    $url = 'https://slack.com/api/chat.postMessage?' . 'token=' . urlencode($cfg_slack_token) . '&channel=' . urlencode($receiver) .'&text=' . urlencode($text) . '&as_user=false&username=' . urlencode($cfg_slack_botname);
    //dp($url);
...
function sso_update() {
    global $cfg_ccp_client_id, $cfg_ccp_client_secret, $cfg_user_agent, $cfg_sql_url, $cfg_sql_user, $cfg_sql_pass, $_SESSION, $_GET;
...
function invite($dbr, $mail) {
    ...
    if ($row = $stm->fetch()) {
	$stm = $dbr->prepare('UPDATE invite SET email = :email, invited_at = :invited_at, email_history = :email_history WHERE character_id = :character_id');
	$stm->bindValue(':character_id', $_SESSION['character_id']);
	$stm->bindValue(':email', $mail);
	...
	$stm = $dbr->prepare('INSERT INTO invite (character_id, character_name, email, email_history, invited_at) VALUES (:character_id, :character_name, :email, "", :invited_at)');
	$stm->bindValue(':character_id', $_SESSION['character_id']);
...
function verify($dbr, $code) {
    ...
    $stm = $dbr->prepare('UPDATE account SET character_id = :character_id, character_name = :character_name, corporation_id = :corporation_id, corporation_name = :corporation_name, alliance_id = :alliance_id, alliance_name = :alliance_name, core_tags = :core_tags, core_groups = :core_groups, core_perms = :core_perms, completed_at = :completed_at WHERE auth_code = :auth_code');
    $stm->bindValue(':character_id', $_SESSION['character_id']);
    $stm->bindValue(':character_name', $_SESSION['character_name']);
...
function pull_character($dbr, $character_id) {
    global $cfg_user_agent, $cfg_core_host, $cfg_core_secret;
    ...
    $stm = $dbr->prepare('UPDATE account SET character_name = :character_name, corporation_id = :corporation_id, corporation_name = :corporation_name, alliance_id = :alliance_id, alliance_name = :alliance_name, faction_id = :faction_id, faction_name = :faction_name, core_tags = :core_tags, core_groups = :core_groups, core_perms = :core_perms, updated_at = :updated_at WHERE character_id = :character_id');
...
function refresher_slack() {
    global $cfg_user_agent, $cfg_slack_token, $cfg_base_url;
	...
	if (!$row) {
	    ...
	    $sti = $dbr->prepare('INSERT INTO account (slack_id, auth_code, created_at) VALUES (:slack_id, :auth_code, :created_at)');
	...
	$stu = $dbr->prepare('UPDATE account SET slack_username = :slack_username, slack_realname = :slack_realname WHERE slack_id = :slack_id');
...
#	    if ($row['slack_realname'] != $row['character_name']) {
		...
		if ($row['name_started_at'] == 0) {
		    dp("REMINDER: New name mismatch detected: " . $row['slack_realname'] . " " . $row['character_name']. "\n");
		...
		if ($row['name_reminder_at'] < time() - 60 * 60 * 12) {
		    ...
		    sendSlack("Your Slack profile name does not match your eve character name. Please fix that: '" . $row['slack_realname'] . "' vs '" . $row['character_name'] . "'", "@" . $row['slack_username']);
		    ...
		    if ($row['name_started_at'] < time() - 60 * 60 * 72 && $row['name_delete_reminder_at'] < time() - 60 * 60 * 24) {
			dp("REMINDER: Notified name mismatch for deletion: @" . $row['slack_username'] . "\n");
			sendSlack('DELETE (wrong name): @' . $row['slack_username'] . ' -- ' . $row['slack_realname'] . ' != ' . $row['character_name'], $cfg_slack_admin);
			$stu = $dbr->prepare('UPDATE account SET name_delete_reminder_at = :time WHERE slack_id = :slack_id');
			$stu->bindValue(':time', time());
			$stu->bindValue(':slack_id', $row['slack_id']);
			if (!$stu->execute()) {
			    dp("ERR; R304\n");
		...
		if ($row['left_reminder_at'] < time() - 60 * 60 * 12) {
		    ...
		    sendSlack('It seems you are no longer blue to Brave. Say your goodbye, you will be kicked soon.', "@" . $row['slack_username']);
		    ...
		    if ($row['left_started_at'] < time() - 60 * 60 * 72 && $row['left_delete_reminder_at'] < time() - 60 * 60 * 24) {
			dp("REMINDER: Notified left for deletion: @" . $row['slack_username'] . "\n");
			sendSlack('DELETE (no verification): @' . $row['slack_username'] . ' -- ' . $row['slack_realname'], $cfg_slack_admin);
			$stu = $dbr->prepare('UPDATE account SET left_delete_reminder_at = :time WHERE slack_id = :slack_id');
			$stu->bindValue(':time', time());
			$stu->bindValue(':slack_id', $row['slack_id']);
			if (!$stu->execute()) {
			    dp("ERR; R308\n");
		...
		sendSlack('You need to verify your account. Please visit ' . $cfg_url_base . ' and enter the following code: ' . $row['auth_code'], "@" . $row['slack_username']);
		...
		if ($row['created_at'] < time() - 60 * 60 * 72 && $row['verify_delete_reminder_at'] < time() - 60 * 60 * 24) {
		    dp("REMINDER: Notified verify for deletion: @" . $row['slack_username'] . "\n");
		    sendSlack('DELETE (no verification): @' . $row['slack_username'] . ' ' . $row['slack_realname'], $cfg_slack_admin);
		    $stu = $dbr->prepare('UPDATE account SET verify_delete_reminder_at = :time WHERE slack_id = :slack_id');
		    $stu->bindValue(':time', time());
		    $stu->bindValue(':slack_id', $row['slack_id']);
		    if (!$stu->execute()) {
			dp("ERR; R311\n");
